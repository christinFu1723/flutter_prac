...
class SearchPage extends StatefulWidget {
  ...
  final String hint;
  const SearchPage({Key key,this.hideLeft=true,this.searchUrl=baseUrl,this.keyword,this.hint});
  @override
...
class _SearchPageState extends State<SearchPage> {
        ...
        body: Column(
          children: [
            ...
            MediaQuery.removePadding(context: context,
                ...
                child:Expanded(
                    flex: 1,
                    child: ListView.builder(itemCount:searchModel?.data?.length??0,itemBuilder: (BuildContext context,int pos){
  ...
  _listViewItem(pos){
    if(searchModel==null||searchModel.data==null||searchModel.data.length<1) return null;
    SearchModelItem item = searchModel.data[pos];
    return GestureDetector(
      onTap: (){
        Navigator.push(context, MaterialPageRoute(builder: (context)=>WebView(url:item.url,title:'详情')));
      },
      child: Container(
